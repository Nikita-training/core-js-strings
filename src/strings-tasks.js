...
function isPalindrome(str) {
  ...
  const straingtArr = cleanStr.split('');
  const reverseArr = straingtArr.reverse();
  return cleanStr === reverseArr.join('');
}
...
function encodeToRot13(str) {
  ...
  arr.forEach((letter) => {
    ...
    if (index < 97 && index > 64)
      index = index + 13 < 65 + 26 ? index + 13 : index + 13 - 26;
    if (index > 96 && index < 123)
      index = index + 13 < 97 + 26 ? index + 13 : index + 13 - 26;
...
function getCardId(value) {
  ...
  return deck.findIndex((n) => n === value);
}
